...
class UserManager(models.Manager):
    ...
    def validate_registration(self, post_data):
        ...
        if len(post_data['last_name']) < 2 or len(post_data['first_name']) < 2:
            errors.append("name fields must be at least 3 characters")
        # check length of name password
        if len(post_data['password']) < 8:
            errors.append("password must be at least 8 characters")
        # check name fields for letter characters
        if not re.match(NAME_REGEX, post_data['first_name']) or re.match(NAME_REGEX, post_data['last_name']) :
...
class Video(models.Model):
    title = models.CharField(max_length=255, default="null")
    link = models.CharField(max_length=255)
    snippet = models.CharField(max_length=255, default="null") ## img link
    done = models.BooleanField(default=False)
    created_at = models.DateTimeField(auto_now_add = True)
    updated_at = models.DateTimeField(auto_now = True)
    video_owner = models.ForeignKey(Todo, related_name = "created_video")
